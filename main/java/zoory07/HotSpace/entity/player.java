...
public void reiniciar() {
    // REINICIAR TODO el estado del jugador
    isGameOver = false; // ✅ DESACTIVAR GAME OVER
    x = 440; // posición inicial
    y = 400; // posición inicial
    
    // Reiniciar animaciones
    if (animacionGameOver != null) {
        animacionGameOver.reset();
    }
    if (animacionCorriendo != null) {
        animacionCorriendo.reset();
    }
    
    // Actualizar posición del hitbox
    int hitboxOffsetX = (int) (width * 0.075);
    int hitboxOffsetY = (int) (height * 0.8);
    if (hitbox != null) {
        hitbox.updatePosition(x + hitboxOffsetX, y + hitboxOffsetY);
    }
    
    System.out.println("Jugador reiniciado - isGameOver = " + isGameOver);
}
